...
class NewVersion extends Command
{
    ...
    {
        parent::__construct();
        $this->product = config('global.product_name');
    }
    ...
    public function handle()
    {
        ...
        $this->comment("\n" . 'run ./upload.sh to upload to ignite server');
    ...
    private function createVersionZip($version)
    {
        ...
        if ($this->product == 'recruit-saas') {
            $this->comment(' Removing user-uploads folders except front-features/feature-* folder for ignite zip version');
