...
trait AppBoot
{
    ...
    {
        return view('vendor.ignite-source.verify_purchase');
    }

    /**
     * @param Request $request
     * @return array
     * Send request on ignite source server to validate
     */
    ...
    public function curl($postData)
    {
        ...
        return [
            'status' => 'success',
            'message' => 'Your purchase code is successfully verified'
        ];

        try {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, config('global.verify_url'));
    ...
    private function getServerData($purchaseCode, $savePurchaseCode = true)
    {
        ...
        // Send request to ignite server to validate the license
        ...
        if ($response && $response['status'] === 'success') {
            ...
            return Reply::successWithData($response['message'] . ' <a href="' . route(config('global.redirectRoute')) . '">Click to go back</a>', ['server' => $response]);
        }
    ...
    public function showInstall()
    {
        try {
            ...
            echo view('vendor.ignite-source.install_message');
            exit(1);
        }
    }
    ...
    public function curlReviewContent($buttonPressedType)
    {
        // Verify purchase
        try {
            $url = str_replace('verify-purchase', 'button-pressed', config('global.verify_url'));
